...
// loosey-goosey. You got some data from an endpoint and you want to pull values
    ...
    I: Index,
{
    ...
    /// Index into a `rocket_config::Value` using the syntax `value[0]` or
    /// `value["k"]`.
    ///
...
where
    I: Index,
{
    /// Write into a `rocket_config::Value` using the syntax `value[0] = ...` or
    /// `value["k"] = ...`.
    ///
